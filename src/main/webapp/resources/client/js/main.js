(function ($) {
  ...
  $(".quantity button").on("click", function () {
    ...
    const total = $(`p[data-cart-total-price]`);

    if (total && total.length) {
      const currentTotal = total.first().attr("data-cart-total-price");
      ...
      total?.each(function (index, element) {
        $(total[index]).text(formatCurrency(newTotal.toFixed(2)) + " đ");

        $(total[index]).attr("data-cart-total-price", newTotal);
      });
    }
  });
  ...
  $(".btnAddToCartHomepage").click(function (event) {
    ...
    if (!isLogin()) {
      $.toast({
        Headers: "Loi thao tac",
        text: "Ban can dang nhap tai khoan",
        position: "top-right",
        icon: "error",
      });
      return;
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $.toast({
          heading: "Gio hang",
          text: "Them san pham vao gio hang thanh cong",
          position: "top-right",
        });
      },
